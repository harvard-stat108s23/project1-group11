...
h5("Listed on these services")
checkboxInput("airbnb", "Airbnb", value = TRUE)
checkboxInput("homeaway", "HomeAway", value = FALSE)
checkboxInput("flipkey", "FlipKey", value = FALSE)
checkboxInput("vrbo", "VRBO", value = FALSE)
checkboxInput("craigslist", "Craigslist", value = FALSE)
#checkboxInput("couch_surfing", "Couch Surfing", value = TRUE)
#checkboxInput("boston_rentals", "Boston Rentals", value = TRUE)

actionButton("update1", "Update results")
```
...
df_map1 = eventReactive(input$update1, {
    df  %>%
      filter(zipcode %in% input$zipcode,
             ...
             flipkey == input$flipkey,
             vrbo == input$vrbo,
             craigslist == input$craigslist) 
  })
...
Maximum Renter Capacity by Zipcode {data-icon="ion-stats-bars"}
=====================================     
...
#color_vars <- c("zipcode", "str_type", "property_type", "total_bedrooms", "rented_bedrooms", "max_renter_cap", "bathrooms", "kitchen", "condo", "airbnb", "flipkey", "vrbo", "craigslist")
...
#pickerInput("size",
      ...
      #      choices = quant_vars,
      #      selected = quant_vars[3], 
      #    multiple = FALSE)

actionButton("update2", "Update results")
```
...
#df_scatter = eventReactive(input$update2, {
 # df %>% 
  #  filter(zipcode %in% input$zipcode) %>%
  #  mutate(x = .data[[input$x]]) %>%
  #  mutate(y = .data[[input$y]]) %>%
  #  mutate(color = .data[[input$color]]) %>%
  #  mutate(size = .data[[input$size]])
#})
  

#x <- eventReactive(input$update2, input$x)
#y <- eventReactive(input$update2, input$y)
#color <- eventReactive(input$update2, input$color)
#size <- eventReactive(input$update2, input$size)


#renderPlotly({
#ggplotly(
 # ggplot(data = df_scatter(), mapping = aes(x = x, y = y, color = color, size = size)) +
 # geom_histogram(size = 2) + 
 # labs(x = x(),
  #     y = y()), 
 # tooltip = c("x", "y", "color", "size")
#)
#  })


df_filtered <- eventReactive(input$update2, {
df %>% filter(property_type %in% input$property_type)
})
